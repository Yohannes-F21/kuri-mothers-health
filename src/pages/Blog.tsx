...
const Blog = () => {
  ...
  useEffect(() => {
    ...
    const fetchBlogs = async () => {
      ...
      try {
        ...
        try {
          // Fallback through Vite proxy path for local setups that accidentally point to direct localhost API.
          ...
          const fallbackMessage =
            (axios.isAxiosError(fallbackError)
              ...
              : undefined) ||
            "Unable to load blog articles right now. Please try again shortly.";

          setErrorMessage(fallbackMessage);
          console.error("Blog fetch failed", { primaryError, fallbackError });
        }
      } finally {
        setIsLoading(false);
      }
  ...
  const publishedBlogs = useMemo(
    () => blogs.filter((blog) => blog.isPublished && blog.category === "blog"),
    [blogs],
  );
      ...
      <section className="relative bg-card pt-32 pb-20 md:pt-40 md:pb-28 overflow-hidden">
        ...
        <div className="section-padding max-w-[1440px] mx-auto relative">
          <div className="max-w-3xl">
            <p className="label-sm mb-4 text-accent">Blog</p>
            <h1 className="heading-xl mb-6">
              Stories, guidance, and trusted motherhood insights.
            </h1>
            <p className="body-lg max-w-2xl">
              Explore expert-backed breastfeeding knowledge, practical care
              tips, and real stories from the Kuri community.
            </p>
          </div>
          ...
          <article className="rounded-2xl overflow-hidden border border-border bg-card shadow-sm">
            <div className="grid lg:grid-cols-2">
              ...
              <div className="p-8 md:p-10 flex flex-col">
                <p className="label-sm text-accent mb-3">Featured Article</p>
                ...
                <div className="mt-auto flex flex-wrap items-center justify-between gap-4 border-t border-border/70 pt-5">
                  ...
                  <span className="inline-flex items-center rounded-full bg-sage-light px-3 py-1 text-xs font-semibold text-primary">
      ...
      <section className="bg-card">
        <div className="section-padding section-y max-w-[1440px] mx-auto">
          <SectionHeading
            ...
            title="Read, learn, and feel supported."
            description="Fresh insights designed to help mothers navigate breastfeeding with confidence."
          />
          ...
          {isLoading ? (
            ...
            <div className="rounded-xl border border-border bg-background p-8 text-center">
              <p className="body-md">No articles found for your search.</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {gridBlogs.map((blog) => (
